...
class OrderController extends Controller
{
    ...
    {
        // dd($request);
        $order = Order::where('orderinfo_id', $id)->update([
        ...
        if ($order) {
            ...
            $email = User::select('email')
                ->where('id', $order->customer->user_id)
                ->first();
            // dd($email);
            Mail::to($email)
            ->send(new SendOrderStatus($order));
        ...
        return redirect()->route('admin.orders')->with('error','update order failed');
    }
}
